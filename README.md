...
- `dedup`：raw hash、canonical hash、simhash、近重复聚类和新颖度惩罚。
- `annotation`：领域、任务类型、复用用途、质量分、价值分和置信度预标注。
...
- `inbox`：专属收件箱、Inbound Message、MIME 文本解析、message id 幂等和邮件网关审计。
- `inbound_gateway`：公开收件网关入口、专用 token 鉴权、网关转发幂等和原始邮件隔离。
- `webhook_ingestion`：浏览器插件、AI 工具、客服系统和内部 Agent trace 的 Webhook Case 入库。
...
- `review_claim`：审核认领、释放和冲突保护，避免多人并发误审。
- `review_samples`：抽检、盲审样本、审核员绩效和高价值样本复核。
- `contract`：Data Contract、Manifest、Quality Report、受控 Artifact 导出和数据出厂门禁。
- `commercial_proof`：商用证明包、Artifact 哈希、授权/审核/内容安全/分账摘要和不含原文的出厂证据。
- `evaluation`：holdout registry、质量回归评测、训练/eval overlap 检查和数据集可用性报告。
- `source_trust`：贡献者来源可信度评分，进入分账权重和风控判断。
- `ledger`：UsageEvent、PayoutEvent、贡献者账本、结算批次和幂等结算状态。
- `reconciliation`：订单、交付、UsageEvent、PayoutEvent、结算批次和发票回款的对账报告。
- `dsr`：数据主体权利请求、授权撤回、未来出厂阻断、历史数据集召回、交付授权吊销和删除证明。
- `sso`：租户级 OIDC/SAML/CAS 配置，作为企业身份接入控制面。
- `content_safety`：内容安全扫描、重要数据候选识别、合规复核阻断和 Case 出厂门禁。
- `compliance_tasks`：ICP、等保、个人信息保护评估、内容安全制度等上线任务与准入检查。
- `provider_configs`：LLM、OCR、ASR、对象存储、支付、发票等供应商配置和健康检查。
- `schema_governance`：迁移版本状态、上线准入阻断和 P0 表结构可观测。
- `cn_region_guard`：中国区独立运营时阻断海外 active 供应商，避免数据处理链路出境。
- `buyer_usage_reports`：买方使用回传，绑定 delivery grant、dataset、order 和审计日志。
- `payout_transfers`：提现批次提交、供应商回单确认和批量 settle 适配层。
- `contributor`：贡献者 dashboard、个人 Case 列表和自助收益账本。
- `enterprise_delivery`：企业客户、数据集交付授权、一次性 token 返回、读取次数/过期/撤销控制和交付审计。
- `commercial_ops`：企业合同、订单、收入确认、租户配额、交付对账和争议冻结/释放。
- `payout_profiles`：贡献者收款资料、KYC/税务/风控状态、账号哈希留存和生产结算门禁。
- `limits`：请求体上限、单节点限流、请求签名、请求追踪和生产护栏。
- `maintenance`：过期 raw quarantine 清理、过期直传会话失效、同步维护 API、异步维护 Job 和管理 CLI。
- `operational_alerts`：上线准入、内容安全、合规复核、队列、供应商、支付和保留策略的运营告警。
