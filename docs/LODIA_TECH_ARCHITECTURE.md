...
架构定位：中国区独立运营的事件驱动数据资产处理平台  
推荐路线：MVP 使用模块化单体 + 异步 worker，规模化后按瓶颈拆分服务

## 0. 品牌与技术模块命名

Lodia 是主品牌，不再把产品定义为单一邮箱系统。技术架构中保留 Inbox 作为低摩擦入口模块，并用 Pipeline、Gold、Ledger、Eval 分别承载数据处理、优质数据集、收益账本和评测能力。

| 产品模块 | 技术含义 |
| --- | --- |
| Lodia Inbox | 邮件、上传、插件、API、MCP 等多入口数据采集层 |
| Lodia Pipeline | 解析、脱敏、去重、标注、质量门禁和 DRL 分级处理链 |
| Lodia Gold | DRL4/DRL5 高质量商用数据集、训练数据集和 gold eval 数据集 |
| Lodia Ledger | CaseID、授权、UsageEvent、PayoutEvent 和审计账本 |
| Lodia Eval | 数据集评测、模型评测、Agent 任务评测和持续质量回流 |
...
Lodia 的核心不是邮件系统，而是高吞吐、可审计、隐私优先的 AI Case 数据资产处理系统。技术架构需要同时满足四类需求：

- 高吞吐采集：支持邮件、上传、插件、API、MCP 等多入口持续写入。
- 异步处理：对解析、OCR、ASR、脱敏、标注、embedding、聚类等重任务异步化。
- 数据治理：原始数据隔离，脱敏后进入业务链路，所有 Case 可追踪、可撤回、可审计。
- 数据质量：自动标注只作为预标注，必须通过 Quality Gate、DRL、Data Contract 和 Quality Report 才能进入商业交付。
...
MVP 使用模块化单体 + 异步 worker：

- Web/API：FastAPI 或 NestJS
- Worker：Celery/RQ 或 BullMQ
- Queue：Redis Stream 或 RabbitMQ
- DB：Postgres + pgvector
- Object Store：S3/R2/OSS
- Search：Postgres full-text 起步
...
- Temporal 作为长流程编排
- Postgres 分区 + 读写分离
- pgvector 升级为 Qdrant/Milvus/Weaviate 可选
- OpenSearch 负责全文检索
...
| Metadata DB | Postgres | 用户、Case、状态、审核、账本 |
| Vector Index | pgvector 或 Qdrant | Case embedding 和聚类检索 |
| Search Index | Postgres FTS 或 OpenSearch | 脱敏 Case 全文检索 |
| Cache | Redis | 队列、限流、短期任务状态 |
| Audit Log | Postgres append-only 或对象存储 | 审计和合规事件 |
| Quality Store | Postgres + Object Store | gate 结果、质量报告、data contract |
...
### ADR-004：Postgres 作为 MVP 主数据源

决策：MVP 使用 Postgres 管理元数据、状态、账本和部分检索。

理由：

- 事务能力强。
- 支持 JSONB、分区、全文检索和 pgvector。
- 足够支撑 MVP 和早期商业化。
