...
- 企业订单和交付记录。
- 账期对账和冻结机制。
- Source Trust Profile、抽检/盲审、Reviewer 绩效和 holdout registry。
- 数据集质量回归评测、训练/eval overlap 检查和 reconciliation report。
- DSR 请求、删除证明、发票回款和租户级 SSO provider 配置。
- P0 completion：Inbox/Webhook 低摩擦导入、公开 Inbound Gateway 鉴权、内容安全/重要数据候选扫描、合规任务、provider config、中国区供应商地域准入、迁移状态检查、买方使用回传、payout transfer 回单、商用证明包、运营告警、维护任务和 launch readiness。
